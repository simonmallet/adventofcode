...
class AdventDay3Test extends TestCase
{
     ...
     * @return int
     */
    private function getShortestSteps(array $nodesA, array $nodesB)
    {
        $stepsCount = null;

        foreach (array_intersect($nodesA, $nodesB) as $key => $value) {
            $stepsA = array_search($value, $nodesA);
            $stepsB = array_search($value, $nodesB);
            if ($stepsCount === null || $stepsA + $stepsB < $stepsCount) {
                $stepsCount = $stepsA + $stepsB + 2; // adding 2 because array index starts at zero instead of one
            }
